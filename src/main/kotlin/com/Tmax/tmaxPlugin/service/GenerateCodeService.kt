...
import com.Tmax.tmaxPlugin.dto.request.GenerateCodeRequest
...
class GenerateCodeService {
    ...
    //javadoc
    fun generateCode(fileExtension: String, code: String): Refactored =
        runCatching {
            val json = JSONObject()
            json.put("model", "mistralai/Mixtral-8x7B-Instruct-v0.1")
            json.put("prompt", GenerateCodeRequest.of(fileExtension, code).messages.first().content)
            json.put("max_new_token", 400)
            ...
            client.newCall(request).execute().use { response ->
                ...
                response.body?.string()?.let { responseBody ->
                    ...
                    if (isSuccess) {
                        ...
                        val cleanedOutput = GenerateCodeRequest.cleanUpResponse(outputText)
                        cleanedOutput
                    } else {
                        // 실패 응답 처리
